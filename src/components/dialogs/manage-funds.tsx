...
const formSchema = z.object({
    ...
    eth: z
        ...
        .refine(val => val === '' || decimalCheck(val, true), {
            message: `ETH must be at least ${MIN_VALUE}`,
        }),
});
...
const ManageFunds = () => {
    ...
    const handleWithdrawal = async () => {
        ...
        try {
            ...
            if (user?.id) {
                ...
                if (res.status === 200) {
                    await createTransaction(user.id, Number(value), TransactionType.WITHDRAWAL, res.txHash, "Withdrawal of " + value + " " + user?.family?.currency, user.family?.id);
                }
    ...
    const transactionCalls = React.useMemo(() => {
        ...
        const tokenInfo = availableTokens.find(
            (token: { contract: string }) => token.contract.toLowerCase() === currencyAddress.toLowerCase()
        ...
        if (activeTab === 'add' && value && Number(value) > 0) {
            ...
            const amountInWei = ethers.parseUnits(value, tokenDecimals);
            ...
            calls.push({
                ...
                args: [familyAddress as `0x${string}`, amountInWei],
            });
        }
                                        ...
                                        <Transaction
                                            ...
                                            onSuccess={async () => {
                                                ...
                                                if (!depositLoggedRef.current && user?.id && user.family?.id && value) {
                                                    ...
                                                    await createTransaction(user.id, Number(value), TransactionType.DEPOSIT, undefined, "Deposit of " + value + " " + user?.family?.currency, user.family.id);
                                                }
