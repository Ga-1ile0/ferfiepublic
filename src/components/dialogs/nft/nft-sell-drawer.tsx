...
export function NFTSellDrawer({ open, onOpenChange, nft }: NFTSellDrawerProps) {
  ...
  useEffect(() => {
    if (open) {
      console.log('Fetching listing status for NFT:', nft, user?.walletAddress);
      const fetchListingStatus = async () => {
        setIsLoadingListing(true);
        setListingError(null);
        setCurrentListing(null);

        console.log('Fetching listing status for NFT:', nft);
        const chain = 'base';
  ...
  const handleListForSale = async () => {
    ...
    console.log('Listing NFT at price:', sellPrice, 'family currency =', ethPriceValue, 'ETH');

    console.log('Listing NFT for sale:', nft, 'at price:', ethPriceValue, 'ETH');
  ...
  const handleCancelListing = async () => {
    ...
    console.log('Attempting to cancel listing:', orderHash);
