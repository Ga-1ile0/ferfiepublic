...
type TokenType = {
  ...
  icon: string;
  image: string;
  contract: string; // Added contract address for exchange rate calculations
};

export function KidPortfolio() {
  // Auth context - get user at component level
  ...
  // State for NFT image modal
  const [selectedNFT, setSelectedNFT] = useState<NFTItem | null>(null);
  const [isImageModalOpen, setIsImageModalOpen] = useState(false);

  // State for NFT sell drawer
  ...
  useEffect(() => {
    const fetchPortfolioData = async () => {
      console.log('Fetching portfolio data for user:', user);
      setIsLoading(true);
      try {
        ...
        if (!user || !user.walletAddress || !user.family || !user.family.currencyAddress) {
          console.log('User not found or missing required data');
          setTokens([]);
          setNfts([]);
          setIsLoading(false);
          return;
        ...
        console.log('Fetching token balances for user:', userAddress);
        console.log('User currency address:', userCurrencyAddress);
        ...
        console.log('Filtered tokens with values:', filteredTokens);
