  ...
  return (
    <Link
      ...
      className={`flex flex-col items-center justify-center gap-1 px-2 py-1 rounded-md transition-colors ${
        isActive ? 'text-primary bg-primary/10' : 'text-black hover:text-primary hover:bg-primary/5'
      }`}
    >
      {icon}
...
export function RoleBasedNavbar({ userId }: { userId?: string }) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 py-2 px-2 z-50">
      <div className="container mx-auto max-w-xl px-4 py-3">
        <motion.div
          className="flex justify-around rounded-xl border-2 border-black bg-[#E87F4E]/90 p-2 shadow-[-5px_5px_0px_#000000] shadow-yellow-700 backdrop-blur-sm"
