    ...
    useEffect(() => {
        const fetchServerData = async () => {
            if (!user?.id || user.role !== 'parent' || !user?.familyId) {
                ...
                setTransactions([]);
                setChildren([]);
                devLog.log('ParentDashboard: User is not a parent or missing family info, skipping server fetch.');
                return;
            }
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="space-y-4">
                            {isLoadingServerData ? (
                                // Skeleton for transactions loading
                                ...
                                Array.from({ length: 3 }).map((_, index) => (
                                    ...
                                    >
                                        ...
                                        <div className="text-right space-y-2">
                                            ...
                                            <Skeleton className="h-3 w-24 bg-muted ml-auto" />
                                        </div>
                                    </div>
                                ))
                            ) : children.length === 0 ? (
