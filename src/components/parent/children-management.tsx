...
import { getNftCollections, getReservoirCollection, addCustomNftCollection } from '@/server/crypto/nft';
...
export function ChildrenManagement() {
    ...
    const [countries, setCountries] = useState<Array<{ code: string; name: string; flagUrl: string }>>([]); // For permission saving
    ...
    const [nftCollections, setNftCollections] = useState<{ hardcoded: HardcodedNft[]; custom: NftCollection[] }>({ hardcoded: [], custom: [] });
                                    ...
                                    <Card>
                                        ...
                                        >
                                            <CardHeader>
                                                ...
                                                {user?.family?.currency && ['CADC', 'IDRX', 'USDC', 'EURC', 'BRZ'].includes(user.family.currency) && (
                                            ...
                                            <CardContent className="space-y-6">
                                                <div className="flex items-center justify-between bg-muted/50 p-4 rounded-lg">
                                                    ...
                                                    <Switch
                                                        ...
                                                        disabled={isSubmitting || !selectedChildId || (user?.family?.currency ? ['CADC', 'IDRX', 'USDC', 'EURC', 'BRZ'].includes(user.family.currency) : false)}
                                                    />
                                                </div>
                    ...
                    {!fetchedCollection ? (
                        ...
                        <div className="space-y-4">
                            <Card>
                                ...
                                <CardContent>
                                    <img src={fetchedCollection.image} alt={fetchedCollection.name} className="w-24 h-24 mx-auto" />
                                    <p className="text-sm text-center mt-2">{fetchedCollection.tokenCount} items</p>
                                </CardContent>
                            </Card>
                            <DialogFooter>
                                <Button variant="outline" onClick={() => setFetchedCollection(null)}>Back</Button>
